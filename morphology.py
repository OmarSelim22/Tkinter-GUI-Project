...
def dilate(image: np.ndarray) -> np.ndarray:
    """Enlarge white parts of the image using max pixel value in the neighborhood"""
    padded = np.pad(image, 1, mode='constant', constant_values=0)
    output = np.zeros_like(image)
    mask = get_mask()
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            ...
            output[i, j] = np.max(neighbourhood * mask)  # Keep grayscale value
    return output

def erode(image: np.ndarray) -> np.ndarray:
    """Shrink white parts of the image using min pixel value in the neighborhood"""
    padded = np.pad(image, 1, mode='constant', constant_values=0)
    output = np.zeros_like(image)
    mask = get_mask()
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            neighbourhood = padded[i:i + 3, j:j + 3]
            output[i, j] = np.min(neighbourhood * mask)  # Keep grayscale value
    return output
...
def internal_boundary(image: np.ndarray) -> np.ndarray:
    """
    Subtract eroded image from original to get internal boundary
    """
    return image - erode(image)

def external_boundary(image: np.ndarray) -> np.ndarray:
    """
    Subtract original image from dilated image to get external boundary
    """
    return dilate(image) - image
